...
class msg_smsgMsg:
    __slots__ = ("num_messages", "bucket_time", "data", "version")
    msgtype = b"smsgMsg"
    ...
    def __repr__(self):
        nm = self.num_messages if self.num_messages is not None else 0
        bt = self.bucket_time if self.bucket_time is not None else 0
        return "msg_smsgMsg(num_messages=%lu, bucket_time=%lu)" % (nm, bt)
